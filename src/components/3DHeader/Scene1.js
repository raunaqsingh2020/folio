import React, { useEffect, useMemo, useRef, useState } from 'react'
import * as THREE from 'three'
import { useFrame } from '@react-three/fiber'
import { Text, Box, useMatcapTexture, Octahedron } from '@react-three/drei'

import useSlerp from './use-slerp'
import useLayers from './use-layers'
import useRenderTarget from './use-render-target'

import { thinFilmFresnelMap } from './ThinFilmFresnelMap'
import { mirrorsData } from './data'

const TEXT_PROPS = {
  textAlign: 'center',
  fontSize: 2,
  font: 'https://fonts.gstatic.com/s/syncopate/v12/pe0pMIuPIYBCpEV5eFdKvtKqBP5p.woff'
}

function Title({ layers, position, ...props }) {
  const group = useRef()
  useEffect(() => {
    group.current.lookAt(0, 0, 0)
  }, [])

  // console.log(position)

  const textRef = useLayers(layers)

  const textColor = getComputedStyle(document.documentElement).getPropertyValue('--text')

  return (
    <group {...props} ref={group}>
      <Text ref={textRef} name="header" depthTest={false} material-toneMapped={false} material-color={textColor} {...TEXT_PROPS}>
        CREATIVE{`\n`}DEVELOPER
      </Text>
    </group>
  )
}

function Mirror({ sideMaterial, reflectionMaterial, args, layers, ...props }) {
  const ref = useLayers(layers)

  useFrame(() => {
    if (ref.current) {
      ref.current.rotation.y += 0.001
      ref.current.rotation.z += 0.01
    }
  })

  return (
    <Box
      {...props}
      ref={ref}
      args={args}
      material={[sideMaterial, sideMaterial, sideMaterial, sideMaterial, reflectionMaterial, reflectionMaterial]}
    />
  )
}

function Mirrors({ envMap, layers, ...props }) {
  // const [thinFilmFresnelMap] = useState(new ThinFilmFresnelMap())
  // const sideMaterial = useResource()
  // const reflectionMaterial = useResource()
  const [sideMaterial, setSide] = useState()
  const [reflectionMaterial, setReflection] = useState()

  return (
    <group name="mirrors" {...props}>
      <meshLambertMaterial ref={setSide} map={thinFilmFresnelMap} color={0xaaaaaa} />
      <meshLambertMaterial ref={setReflection} map={thinFilmFresnelMap} envMap={envMap} />
      {/* <meshBasicMaterial ref={setSide}/>
      <meshBasicMaterial ref={setReflection}/> */}
      {mirrorsData.mirrors.map((mirror, index) => (
        <Mirror
          key={`mirror-${index}`}
          layers={layers}
          {...mirror}
          name={`mirror-${index}`}
          sideMaterial={sideMaterial}
          reflectionMaterial={reflectionMaterial}
        />
      ))}
    </group>
  )
}

function TitleCopies({ layers }) {
  const vertices = useMemo(() => {
    const y = new THREE.IcosahedronGeometry(8)
    // console.log(y.attributes["position"].array)
    var vertices = []
    let pos = y.attributes.position; 
    for (var i = 0; i < 12; i++) {
      vertices.push(new THREE.Vector3().fromBufferAttribute(pos, i));
    }
    
    // return y.vertices
    return vertices
  }, [])

  return (
    <group name="titleCopies">
      {vertices.map((vertex, i) => (
        <Title name={'titleCopy-' + i} position={vertex} layers={layers} />
      ))}
    </group>
  )
}

function Scene({ theme }) {
  const [cubeCamera, renderTarget] = useRenderTarget()
  const group = useSlerp()

  const [matcapTexture] = useMatcapTexture('C8D1DC_575B62_818892_6E747B', 1024)

  const bgColor = getComputedStyle(document.documentElement).getPropertyValue('--mirrors')

  console.log(theme)

  return (
    <>
      <group name="sceneContainer" ref={group}>
        <Octahedron layers={[11]} name="background" args={[20, 4, 4]} position={[0, 0, -5]}>
          {theme === "light" ? (
            <meshMatcapMaterial matcap={matcapTexture} side={THREE.BackSide} transparent opacity={1} color="#A0F"/>
          ) : (
            <meshMatcapMaterial matcap={matcapTexture} side={THREE.BackSide} transparent opacity={1} color="#000"/>
          )}
        </Octahedron>
        <cubeCamera
          layers={[11]}
          name="cubeCamera"
          ref={cubeCamera}
          args={[0.1, 100, renderTarget]}
          position={[0, 0, 5]}
        />
        <Title name="title" position={[0, 0, -200]}/>
        <TitleCopies layers={[11]}/>
        <Mirrors layers={[0, 11]} envMap={renderTarget.texture} />
      </group>
    </>
  )
}

export default Scene
